...
final class GrpcCodes
{
     ...
     *
     * E.g., if an "rmdir" fails because the directory is non-empty, FAILED_PRECONDITION should be returned since
     ...
     * that is not in the range [0,2^32-1], but it will generate OUT_OF_RANGE if asked to read from an offset
     * past the current file size.
     *
     * There is a fair bit of overlap between FAILED_PRECONDITION and OUT_OF_RANGE.
     * We recommend using OUT_OF_RANGE (the more specific error) when it applies so that
     * callers who are iterating through a space can easily look for an OUT_OF_RANGE error to detect when they are done.
     */
